...
import MedicalAssistanceDialog from '../MedicalAssistanceDialog';
...
const ProfileTable = (props) => {
  ...
  const [isOpenAssistanceDialog, setIsOpenAssistanceDialog] = useState(false);
  const classes = useStyles();

  const location = useLocation();
  const history = useHistory();
  
  const handleCloseAssistanceDialog = (assistanceId) => {
      setIsOpenAssistanceDialog(false);
      
      if(assistanceId) {
        history.push(`./${assistanceId}`);
      }
  };
  const openAssistanceDialog = () => setIsOpenAssistanceDialog(true);

  let { moId, assistanceId } = useParams();
  moId = Number(moId);
  assistanceId = Number(assistanceId);
  const nodeId = useSelector(selectedNodeIdSelector);
  const profile = useSelector((store) => medicalAssistanceTypeByIdSelector(store, assistanceId));
  ...
  console.log('Polycl Change FapAssistance 0');
  if (isLoading || !indicators || !moDepartments || periodIds.length === 0 || Object.keys(periods).length === 0 || !profile || !moId) {
      return (
        <div>
          <LinearProgress />
          <Link component="button" variant="body2" onClick={openAssistanceDialog}>(выбрать показатель)</Link>
        </div>
      );
  }
  console.log('Polycl Change FapAssistance 1');

  const indicatorLength = indicators.length;

  return (
    <div>
      <MedicalAssistanceDialog open={isOpenAssistanceDialog} onClose={handleCloseAssistanceDialog} />
      
      {profile.name} <Link component="button" variant="body2" onClick={openAssistanceDialog}>(выбрать показатель)</Link>
