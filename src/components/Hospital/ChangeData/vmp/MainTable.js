...
import { selectedNodeIdSelector, careProfilesForNodeIsLoadingSelector, indicatorsForSelectedNodeSelector, indicatorForNodeIsLoadingSelector, careProfilesArrForSelectedNodeSelector } from '../../../../store/nodeData/nodeDataSelectors';
...
const MainTable = (props) => {
  ...
  const careProfilesForNodeIsLoading = useSelector((store) => careProfilesForNodeIsLoadingSelector(store, nodeId));
  const indicatorForNodeIsLoading = useSelector((store) => indicatorForNodeIsLoadingSelector(store, nodeId));
  const isLoading = changeIsLoading || careProfilesForNodeIsLoading || indicatorForNodeIsLoading;
  
  console.log('M0');
  if (isLoading || !indicators || !careProfiles || mo.length === 0 || periodIds.length === 0) {
